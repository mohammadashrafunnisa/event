    ...
    try:
        ...
        if request.method=='POST':
            ...
            if npassword==cpassword:
                cursor=mydb.cursor(buffered=True)
                cursor.excute('update users set password=%s where email=%s',[npassword,data])
                mydb.commit()
                cursor.close()
...
def vlogout():
    if session.get('vendor'):
        ...
        return redirect(url_for('vlogin'))
    return redirect(url_for('vlogin'))
@app.route('/vforget',methods=['GET','POST'])
def vforgot():
    if request.method=='POST':
        email=request.form['id']
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select count(*) from vsignup where email=%s',[email])
        count=cursor.fetchone()[0]
        cursor.close()
        try:
            if count != 1:
...
def vverify(token):
    try:
        ...
        if request.method=='POST':
            ...
            if npassword==cpassword:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('update signup set password=%s where email=%s',[npassword,data])
                mydb.commit()
                cursor.close()
                flash('password has been updated')
                return redirect(url_for('login'))
